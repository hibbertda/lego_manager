...
class BricksetAPI:
    def __init__(self, api_key: str):
        ...
        self._userhash_cache: dict[tuple[str, str], str] = {}
    ...
    def get_instructions(self, set_id: int) -> Optional[list[dict[str, Any]]]:
        ...
        params = {"setID": set_id, "apikey": self.api_key}
        try:
    ...
    def get_sets(self, set_number: str, userhash: Optional[str] = None) -> Optional[dict[str, Any]]:
        ...
        try:
            ...
            logger.exception("Failed to fetch set information for set number: %s", set_number)
            return None
        return response.json()

    def download_images(self, set_data: dict[str, Any], base_dir: str = "sets") -> None:
    ...
    def download_instructions(self, set_data: dict[str, Any], base_dir: str = "sets") -> None:
        set_number = set_data.get("number")
        if not set_number:
            return
        ...
        core_instructions = [
            instr for instr in (set_data.get("instructions") or [])
        ...
        for url in instruction_urls:
            ...
            try:
                ...
                logger.exception("Failed to download instruction: %s", url)
                continue
            with open(local_path, "wb") as f:
                f.write(response.content)
            local_instruction_paths.append(local_path)
            logger.info("Downloaded instruction: %s", local_path)

        set_data["local_instructions"] = local_instruction_paths

    def get_combined_data(self, set_number: str, userhash: Optional[str] = None) -> Optional[dict[str, Any]]:
        ...
        self.download_images(set_info["sets"][0])
        self.download_instructions(set_info["sets"][0])

        set_dir = os.path.join("sets", set_number)
