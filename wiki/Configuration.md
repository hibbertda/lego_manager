...
| `TRUST_PROXY_HEADERS` | No | `1` | Trust `X-Forwarded-Proto`/`Host`/`Port` from a reverse proxy (via Werkzeug's `ProxyFix`), so generated absolute URLs — notably the OIDC `redirect_uri` — use the proxy's public `https://` domain instead of the internal gunicorn bind address. Defaults on since this app is expected to always run behind a reverse proxy; set to `0` only if gunicorn is ever reachable directly by clients with no proxy in front (trusting these headers from an untrusted client would let them spoof scheme/host). |
